...
WHERE DEPARTMENT.DEPT_ID = EMPLOYEE.DEPT_CODE
ORDER BY DEPT_CODE;
...
SELECT EMP_ID, 
       ...
       JOB_NAME
FROM EMPLOYEE E
INNER JOIN JOB USING (JOB_CODE); -- 괄호 생략 가능. USING에 있는 컬럼은 별칭이나 테이블을 넣어주면 안된다.
...
SELECT E.EMP_ID, 
       ...
       E.SALARY
FROM EMPLOYEE E
INNER JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE) -- WHERE 절의 문구를 ON 절에도 넣어도 되는데 WHERE로 넣어야 함
WHERE J.JOB_NAME = '대리';
...
FROM DEPARTMENT D 
INNER JOIN EMPLOYEE E ON(E.DEPT_CODE = D.DEPT_ID)
WHERE NVL(BONUS, 0) > 0;
...
SELECT E.EMP_NAME, 
       ...
       E.SALARY*12
FROM EMPLOYEE E
LEFT /*OUTER*/ JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID) -- 왼쪽에 있는 테이블을 다 보여주겠다. DEPARTMENT 테이블에서 가져와야 할 값은 다 NULL
...
-- EMPLOYEE, DEPARTMET, LOCATION 테이블을 다중 JOIN 하여 사번, 직원명, 부서명, 지역명을 조회
...
-- ANSI 구문
-- 사번 : EMPLOYEE, 직원명 : EMLOYEE, 부서명 : DEPARTMENT, 지역명 : LOCATION, 국가명 : LOCATION
SELECT E.EMP_ID, 
       E.EMP_NAME, 
       D.DEPT_TITLE, 
       L.LOCAL_NAME, 
...
WHERE E.DEPT_CODE = D.DEPT_ID AND L.LOCAL_CODE = D.LOCATION_ID AND L.NATIONAL_CODE = N.NATIONAL_CODE;

-- 2. 사번, 직원명, 부서명, 직급명, 지역명, 국가명, 급여 등급을 조회 
-- ANSI 구문
SELECT E.EMP_ID, 
       E.EMP_NAME, 
       D.DEPT_TITLE, 
...
WHERE E.DEPT_CODE = D.DEPT_ID AND E.JOB_CODE = J.JOB_CODE AND L.LOCAL_CODE = D.LOCATION_ID AND L.NATIONAL_CODE = N.NATIONAL_CODE AND E.SALARY BETWEEN S.MIN_SAL AND S.MAX_SAL;

---------------------------------------------------------- 
-------------------- 종합 실습 문제 ------------------------ 
-- 1. 직급이 대리이면서 ASIA 지역에서 근무하는 직원들의 사번, 직원명, 직급명, 근무 지역, 급여를 조회
-- ANSI 구문
SELECT E.EMP_ID, E.EMP_NAME, L.LOCAL_NAME, E.SALARY
FROM EMPLOYEE E
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE)
JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
JOIN LOCATION L ON (D.LOCATION_ID = L.LOCAL_CODE)
...
WHERE E.JOB_CODE = J.JOB_CODE AND E.DEPT_CODE = D.DEPT_ID AND L.LOCAL_CODE = D.LOCATION_ID AND J.JOB_NAME = '대리' AND SUBSTR(L.LOCAL_NAME, 1, 4) = 'ASIA';

-- 2. 70년대생 이면서 여자이고, 성이 전 씨인 직우너들의 직원명, 주민 등록 번호, 부서명을 조회
-- ANSI 구문
SELECT E.EMP_NAME, E.EMP_NO, D.DEPT_TITLE
FROM EMPLOYEE E
JOIN DEPARTMENT D ON(E.DEPT_CODE = D.DEPT_ID) 
WHERE SUBSTR(E.EMP_NO, 8, 1) = '2' AND SUBSTR(E.EMP_NAME, 1, 1) = '전';

-- 오라클 구문
SELECT E.EMP_NAME, E.EMP_NO, D.DEPT_TITLE
FROM EMPLOYEE E, DEPARTMENT D
WHERE E.DEPT_CODE = D.DEPT_ID AND SUBSTR(E.EMP_NO, 8, 1) = '2' AND SUBSTR(E.EMP_NAME, 1, 1) = '전';
...
WHERE E.DEPT_CODE = D.DEPT_ID AND L.LOCAL_CODE = D.LOCATION_ID AND L.LOCAL_CODE = D.LOCATION_ID AND E.BONUS IS NOT NULL;
-- 4. 한국과 일본에서 근무하는 직원들의 직원명, 부서명, 근무 지역, 근무 국가를 조회
-- ANSI 구문
SELECT E.EMP_NAME, E.DEPT_CODE, L.LOCAL_NAME, N.NATIONAL_NAME 
FROM EMPLOYEE E
JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
JOIN LOCATION L ON (D.LOCATION_ID = L.LOCAL_CODE)
JOIN NATIONAL N ON (L.NATIONAL_CODE = N.NATIONAL_CODE)
...
SELECT E.EMP_NAME, E.DEPT_CODE, L.LOCAL_NAME, N.NATIONAL_NAME 
FROM EMPLOYEE E, DEPARTMENT D, LOCATION L, NATIONAL N
WHERE E.DEPT_CODE = D.DEPT_ID AND D.LOCATION_ID = L.LOCAL_CODE AND L.NATIONAL_CODE = N.NATIONAL_CODE AND (N.NATIONAL_NAME = '한국' OR N.NATIONAL_NAME = '일본');

-- 5. 각 부서별 평균 급여를 조회하여 부서명, 평균 급여를 조회 (단, 부서 배치가 안된 사원들의 평균도 출력)
-- ANSI 구문
SELECT NVL(D.DEPT_TITLE, '부서없음'), ROUND(AVG(E.SALARY)) 
FROM EMPLOYEE E
LEFT JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
...
-- ANSI 구문
SELECT NVL(D.DEPT_TITLE, '부서없음'), TO_CHAR(SUM(E.SALARY), 'FML99,999,999') 
FROM EMPLOYEE E
JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
GROUP BY D.DEPT_TITLE
...
-- 오라클 구문
SELECT D.DEPT_TITLE, TO_CHAR(SUM(E.SALARY), 'FML99,999,999') 
FROM EMPLOYEE E, DEPARTMENT D
WHERE E.DEPT_CODE = D.DEPT_ID
GROUP BY D.DEPT_TITLE
HAVING SUM(E.SALARY) >= 10000000;
...
-- 이때 구분에 해당하는 값은 아래와 같이 조회되도록 하시요.
--  급여 등급이 S1, S2인 경우 '고급'
--  급여 등급이 S3, S4인 경우 '중급'
--  급여 등깁이 S5, S6인 경우 '초급'
-- ANSI 구문
SELECT E.EMP_ID,
       ...
       CASE WHEN (S.SAL_LEVEL = 'S1' OR S.SAL_LEVEL = 'S2') THEN '고급' 
            ...
            WHEN (S.SAL_LEVEL = 'S5' OR S.SAL_LEVEL = 'S6') THEN '초급'
        END AS "구분"
FROM EMPLOYEE E
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE)
JOIN SAL_GRADE S ON (E.SALARY BETWEEN S.MIN_SAL AND S.MAX_SAL)
...
FROM EMPLOYEE E
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE)
WHERE E.BONUS IS NULL AND (J.JOB_CODE = 'J4' OR J.JOB_CODE = 'J7');

-- 오라클 구문
SELECT E.EMP_NAME, D.DEPT_TITLE, E.SALARY
FROM EMPLOYEE E, DEPARTMENT D
WHERE E.DEPT_CODE = D.DEPT_ID AND E.BONUS IS NULL AND (E.DEPT_CODE = 'J4' OR E.DEPT_CODE = 'J7');

-- 9. 해외영업팀에 근무하는 직원들의 직원명, 직급명, 부서코드, 부서명을 조회
-- ANSI 구문
...
SELECT E.EMP_NAME, J.JOB_NAME, E.DEPT_CODE, D.DEPT_TITLE 
FROM EMPLOYEE E, DEPARTMENT D, JOB J
WHERE SUBSTR(D.DEPT_TITLE, 1, 4) = '해외영업' AND E.DEPT_CODE = D.DEPT_ID AND E.JOB_CODE = J.JOB_CODE ;
...
SELECT E.EMP_ID, E.EMP_NAME, J.JOB_NAME
FROM EMPLOYEE E, JOB J
WHERE E.EMP_NAME LIKE '%형%' AND E.JOB_CODE = J.JOB_CODE;
---------------------------------------------------------- 
